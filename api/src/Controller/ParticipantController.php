...
class ParticipantController extends AbstractController
{
    ...
    public function update(int $id, Request $request): JsonResponse
    {
        ...
        if (isset($data['paye'])) $participant->setPaye((bool) $data['paye']);
    ...
    // PATCH /api/participants/{id}/paiement — Basculer le statut de paiement
    #[Route('/{id}/paiement', name: 'participant_paiement', methods: ['PATCH'])]
    public function paiement(int $id): JsonResponse
    {
        $participant = $this->participantRepository->find($id);
        if (!$participant) {
            return $this->json(['message' => 'Participant non trouvé'], Response::HTTP_NOT_FOUND);
        ...
        $participant->setPaye(!$participant->isPaye());
        $this->em->flush();

        return $this->json([
            'message' => $participant->isPaye() ? 'Paiement confirmé' : 'Paiement annulé',
            'paye' => $participant->isPaye(),
        ]);
    }
